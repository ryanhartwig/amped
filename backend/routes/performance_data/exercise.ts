...
exercise.get('/:performed_routine_id', async (req, res) => {
  ...
  if (!performed_routine_id) return res.status(400).send('No performed exercise id provided');
  const response = await db.query('select * from performed_exercise where performed_routine_id = $1', [performed_routine_id]);

  if (!response.rowCount) return res.status(404).send('No exercise data for provided routine id');
...
exercise.post('/new', async (req, res) => {
  ...
  if (!response.rowCount) return res.status(500).send('Could not create exercise data');
...
exercise.put('/:id', async (req, res) => {
  ...
  if (!response.rowCount) return res.status(500).send('Could not update exercise data');
...
exercise.delete('/:id', async (req, res) => {
  ...
  if (!response.rowCount) return res.status(404).send('No exercise data with provided id exists');
