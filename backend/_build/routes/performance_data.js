...
const data = (0, express_promise_router_1.default)();
/* Get all data for user */
data.get('/:user_id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
data.post('/new', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id, routine_id, user_id, duration, start_date, notes, energy } = req.body;
    const params = [id, routine_id, user_id, duration, start_date, notes, energy];
  ...
  `, params);
    if (!response.rowCount)
        return res.status(500).json('Could not create routine');
    res.status(201).json(response.rows[0]);
}));
/* Edit an existing routine */
data.put('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    const patch = req.body;
    const select = yield db_1.default.query('select * from routine where id = $1', [id]);
    const existing = select.rows[0];
    if (!existing)
        return res.status(404).json('Could not find routine with given id');
  ...
  `, [id, ...params]);
    if (!response.rowCount)
        return res.status(500).json('Could not update routine');
    res.status(200).json(response.rows[0]);
}));
/* Delete an existing routine */
data.delete('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { id } = req.params;
    if (!id)
    ...
    return res.status(200).json(`Successfully deleted routine: '${response.rows[0].name}'`);
}));
